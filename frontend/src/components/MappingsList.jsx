...
export default function MappingsList({ mappings, onEdit, onDelete }) {
  const sorted = useMemo(
    () => [...mappings].sort((a, b) => a.control_id.localeCompare(b.control_id)),
    [mappings]
  );

  if (sorted.length === 0) {
    return (
      <div data-testid="mappings-list" className="text-xs text-neutral-500 font-mono py-3 text-center">
        no mappings yet — click any control or hit MIDI LEARN
      </div>
    );
  }

  return (
    <ul data-testid="mappings-list" className="space-y-1 max-h-72 overflow-y-auto">
      {sorted.map((m) => (
        <li
          key={m.id}
